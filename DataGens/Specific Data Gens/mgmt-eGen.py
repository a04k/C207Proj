...
def genPerson():
    ...
    person = {
        ...
        'birthdate': datetime.date(random.randint(1965, 1985), random.randint(1, 12), random.randint(1, 28)),
        'gender': gender
    }
    return person

personData = [genPerson() for _ in range(15)]

employeeData = [{'SSN': person['SSN'], 'Salary':random.choice([80000,95000,85000,90000])} for person in personData]
mgmtemployeeData = [{'SSN': person['SSN'], 'Management_Name': "Sport"} for person in personData]
...
mgmtemployee_insert_statement = "INSERT INTO Management_Employee (Employee_SSN,Management_Name) VALUES\n "

for mgmtemployee in mgmtemployeeData:
  values = f'({mgmtemployee["SSN"]},{mgmtemployee["Management_Name"]}),\n'
  employee_insert_statement += values
...
with open('sportmgmt.empData.txt', 'w') as file:
